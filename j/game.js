...
// ================= FIT MAP =================
const islandCoords = islands.map(i => i.coords);
map.fitBounds(L.latLngBounds(islandCoords).pad(0.1));
...
// ================= DRAW ISLANDS =================
islands.forEach(i => {
  const m = L.circleMarker(i.coords, { radius: 8, color: '#333', fillColor: '#fff', fillOpacity: 1 }).addTo(map);
  i.marker = m;
...
rollBtn.addEventListener('click', () => {
  ...
  movesRemaining = currentDice;
  if (thief.doubleMove) {
    movesRemaining *= 2;
    thief.doubleMove = false;
  }

  diceResultDiv.innerText = movesRemaining;
  highlightMoves(getReachableIslands(thief.island, movesRemaining));
});
...
function movePlayer(player, island, isPolice = false) {
  ...
  animateMove(player.marker, start, end, 400, () => {
    ...
    if (!isPolice) checkCatch();

    if (!isPolice && player === thief) {
      movesRemaining--;
      if (movesRemaining > 0) {
        highlightMoves(getReachableIslands(thief.island, movesRemaining));
      } else {
...
function handleIslandClick(island) {
  ...
  if (!getReachableIslands(thief.island, movesRemaining).includes(island)) {
